import menublack from './menu_black.jpg';
import drone1 from './drone1.jpg';
import menuwhite from './menu_white.jpg';
import skytllogo from './SkyTLLogo.png';
import close from './close.svg';
import discount from './Discount.svg';
import city from './city.jpg';
import citynyc from './citynyc.jpg';
import wildfirelight from './wildfirelight.jpg';
import wildfireport from './wildfireport.jpg';
import faalogo from './faa-logo.png';
import nasalogo from './NASA.png';
import noaalogo from './noaa_logo.png';
import FAA from './federal-aviation-admin.png';
import GeorgiaTech from './GT.png';
import Bluenest from './Bluenest.png';
import ERAU from './ERAU.png';
import FSU from './FSU.png';
import NationalInstitueofAerospace from './NationalInstitueofAerospace.png';
import InsuranceInstitute from './InsuranceInstitueForBusinessHomeSecurity.png';
import StruWeather from './StruWeather.png';
import TampaBayWave from './TampaBayWave.png';
import TBTI from './TBTI.png';
import VentureLab from './VentureLab.png';
import SkytlLogoBlack from './skyTLLogoBlack.png';
import WildfireDefense from './WildfireDefenseSystems.png';
import awsicon from './aws-icon.png';
import wildfiremp4 from './forest-fire-burning.mp4';
import agriculturemp4 from './agriculture.mp4';
import wildfire_up_close from './wildfire_up_close.jpg';
import agriculture_drone from './agriculture_drone.jpg';
import pollution from './pollution.jpg';
import crane from './crane.jpg';
import wildfire_close from './wildfire_close.jpg';
import water_quality from './water_quality.jpg';
import lidar_of_london from './Lidar_3D_Model_of_London.jpg';
import wildfire_forest_blaze from './wildfire_forest_blaze.jpg';
import drone_6021114_1280 from './drone_6021114_1280.jpg';
import Red_flag_Sleeper from './Red_flag_Sleeper.jpg';
import us_air_force from './us_air_force.png';
import fire from './fire.jpg';
import F22 from './F22.jpg';
import teaming_aircraft from './teaming_aircraft.jpg';
import wildfire_wui from './wildfire_wui.jpg';
import water_pollution from './Water-pollution.jpg';

export {
  menublack,
  menuwhite,
  skytllogo,
  close,
  drone1,
  discount,
  city,
  citynyc,
  wildfirelight,
  wildfireport,
  faalogo,
  nasalogo,
  noaalogo, 
  FAA,
  GeorgiaTech,
  Bluenest,
  ERAU,
  NationalInstitueofAerospace,
  InsuranceInstitute,
  StruWeather,
  TampaBayWave,
  F22,
  teaming_aircraft,
  TBTI,
  wildfire_wui,
  water_pollution,
  fire,
  VentureLab,
  SkytlLogoBlack,
  WildfireDefense,
  FSU,
  awsicon,
  drone_6021114_1280,
  wildfiremp4,
  agriculturemp4,
  wildfire_forest_blaze,
  Red_flag_Sleeper,
  wildfire_up_close,
  agriculture_drone,
  pollution,
  crane,
  us_air_force,
  wildfire_close,
  water_quality,
  lidar_of_london,
};